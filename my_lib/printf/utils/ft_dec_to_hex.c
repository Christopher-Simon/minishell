...
/*   Updated: 2022/06/01 15:05:24 by chsimon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_ptf_putnbr_base(char *str, unsigned long n,
char *base, int count)
{
	unsigned long	div;
	unsigned long	mod;
	...
	mod = n % i;
	if (div != 0)
		ft_ptf_putnbr_base(str, div, base, count - 1);
	str[count] = base[mod];
	return (str);
}
...
static char	*to_base(unsigned long nbr, char *base)
{
	...
	if (!str)
		return (0);
	str[i] = '\0';
	ft_ptf_putnbr_base(str, n, base, i - 1);
	return (str);
}
