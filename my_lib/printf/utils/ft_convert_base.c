...
/*   Updated: 2022/06/01 15:01:15 by chsimon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_cb_putnbr_base(char *str, unsigned long int n,
char *base, int count)
{
	unsigned int	div;
	unsigned int	mod;
	...
	mod = n % i;
	if (div != 0)
		ft_cb_putnbr_base(str, div, base, count - 1);
	str[count] = base[mod];
	return (str);
}
...
char	*to_base(unsigned int nbr, char *base)
{
	...
	if (!str)
		return (0);
	str[i] = '\0';
	ft_cb_putnbr_base(str, n, base, i - 1);
	return (str);
}
